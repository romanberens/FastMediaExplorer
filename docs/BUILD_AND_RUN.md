...
You can change the current root folder in two equivalent ways:

1. Drag and drop a folder onto the main app window.
2. Use `File -> Open folder...` (or `Ctrl+O`) and choose a folder in the system dialog.

Notes:
- After root change, TreeView root is replaced and scan starts immediately.
- Non-folder drops are rejected with a clear warning message.
- Async scan, progress bar, status bar, and overlay remain active during scan.
